import { Injectable } from '@angular/core';
import { Http, Response } from '@angular/http';
import 'rxjs/add/operator/map';
import { Observable } from 'rxjs/Observable';
import { HttpClient } from '@angular/common/http';
import { UrlApi } from '../url-api';


@Injectable()
export class ReasontransferService {
  url_api = new UrlApi();
  constructor(private http: HttpClient) { }
  getTypeTransferReason(): Observable<any> {
    return this.http.get(this.url_api.get_url_api() + 'api/Transferreason/getTypeTransferReason');
  }
  getTransferActive(): Observable<any> {
    return this.http.get(this.url_api.get_url_api() + 'api/Transferreason/getTransferActive');
  }
  get_transferreason(): Observable<any> {
    return this.http.get(this.url_api.get_url_api() + 'api/Transferreason');
  }
  add_transferreason(data: any): Observable<any> {
    return this.http.post(this.url_api.get_url_api() + 'api/Transferreason', data);
  }
  edit_transferreason(id: any, data: any): Observable<any> {
    return this.http.put(this.url_api.get_url_api() + 'api/Transferreason/' + id, data);
  }
  delete_transferreason(id: any): Observable<any> {
    return this.http.delete(this.url_api.get_url_api() + 'api/Transferreason/' + id);
  }
  delete_oktransferreason(id: any): Observable<any> {
    return this.http.get(this.url_api.get_url_api() + 'api/Transferreason/delete/' + id);
  }
}
